...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if (getLeftX() < r.getRightX() && getRightX() > r.getLeftX() && getBottomY() > r.getTopY() && getTopY() < r.getBottomY()) {
        return true;
    }
...
void Rect::draw() const {
    ...
    glVertex2i(center.x - (size.width / 2.0),
               center.y - (size.height / 2.0)); //Top left
    glVertex2i(center.x - (size.width / 2.0),
               center.y + (size.height / 2.0)); //Bottom left
    glVertex2i(center.x + (size.width / 2.0),
               center.y + (size.height / 2.0)); //Bottom right
    glVertex2i(center.x + (size.width / 2.0),
               center.y - (size.height / 2.0)); //Top right
